...
> These are the questions used in the supplied evaluation notebook. If the approved proposal uses different wording, replace this section with the proposal wording exactly as approved.

## Project structure

```text
cse437-30-day-repeat-purchase-prediction-<group-number>/
├── README.md
├── requirements.txt
├── .gitignore
│
├── data/
│   ├── raw/
│   │   └── <downloaded Online Retail II workbook>
│   ├── processed/
...
The exact versions used for submission should be recorded in `requirements.txt`.
...
The report follows the supplied CSE437 Project Report Template and keeps the required section numbering.

## Repository rules

- Keep raw source data under `data/raw/`.
- Keep generated cleaning/preprocessing outputs under `data/processed/`.
- Keep the original source file unchanged.
- Use relative paths only; do not use machine-specific absolute paths.
- Run the notebooks top-to-bottom on a fresh kernel before submission.
- Save notebook outputs.
- Each group member should commit from their own account and contribute throughout the project.
